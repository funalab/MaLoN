...
import seaborn
import utils
from single_view_attention_lstm_based_regressor import SingleViewAttentionLSTMBasedRegressor
...
def trainer(inputs):
    ...
    model = SingleViewAttentionLSTMBasedRegressor(input_size=n_regulators,
    ...
    for epoch in range(n_epoch):
        ...
        # Regression
        exp_predicted_i, exp_predicted_source_i, reg_score_i = model.forward(x=exp_source)

        # Get loss
        mse_loss = mse(exp_predicted_i, exp_target)
        diff_loss = utils.get_differential_loss(exp_predicted_i)
        total_loss = w_mse * mse_loss + w_diff * diff_loss
